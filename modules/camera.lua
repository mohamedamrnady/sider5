		...
		if entry then
			...
			if base then
				...
				if entry.format ~= "" then
					...
					if log_it then
						log(string.format("%s: changed at %s: %s --> %s", name, hex(addr), old_value, new_value))
					end
				else
